...
contract StorageTree {
    ...
    internal view returns(bytes32[160] memory defaultHashes) {
        assembly {
            extcodecopy(sload(defaultHashStorage.slot), add(defaultHashes, 0x20), 0, mul(160, 0x20))
        }
    }
   ...
   */
  function verifyAndUpdate(bytes32 _root, bytes memory _proveData, bytes memory _setData, uint256 _path, bytes32[] memory _siblings)
